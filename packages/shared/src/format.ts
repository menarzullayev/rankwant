...
export function formatDay(kit: DateKit, value: string): string {
  //: `noUncheckedIndexedAccess` — `split("-")` natijasi `string[]` emas,
  //: `(string | undefined)[]`. Regex bilan tekshirilgandan keyin uchta
  //: bo'lak borligi KAFOLATLANGAN, lekin tip buni bilmaydi; shuning
  //: uchun tayinlashda ochiq tekshiriladi (jim `!` emas — u yolg'onni
  //: yashirardi, bu esa `NaN` ni ko'rsatadi).
  const parts = (/^\d{4}-\d{2}-\d{2}$/.test(value) ? value : isoDay(value)).split("-");
