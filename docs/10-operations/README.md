...
| Arxiv ro'yxat keshi | **2026-09-20 HITL problems-al**: mehmon GET `/problems` query'siz, til `Accept-Language` (uz majburiy emas). `?page=`/`?lang=`/`sessionid` chiqariladi. Worker `problems*` → `problems/*`. CF Cache Rule Eligible + origin TTL: `tools/cf-guest-cache-rule.json`, `python tools/cf_guest_cache_apply.py` |
