...
| Bosh sahifa keshi | **2026-09-19**: mehmon GET `/`, `/login`, `/register`, `/terms`, `/privacy` (sessiya/`rw_locale`/`rw:markup` yo'q, `rw_exp` yozilmaydi; tan `uz`). Origin `public, s-maxage=30, stale-while-revalidate=86400`; kirgan `private, no-store`. Worker `l*` olib tashlangan (`leaderboard*`/`learn*` qoladi). Kod `apps/web/src/lib/home-cache.ts` |
| Arxiv ro'yxat keshi | **2026-09-20 HITL problems-al**: mehmon GET `/problems` query'siz, til `Accept-Language` (uz majburiy emas). Origin `Vary` + CF Cache Rule `vary.normalize`. `?page=`/`?lang=`/`sessionid` chiqariladi. Worker `problems*` → `problems/*`. JSON: `tools/cf-guest-cache-rule.json`, `python tools/cf_guest_cache_apply.py` |
...
| Vary `Accept-Language` | **2026-09-20 HITL cf-transform**: Next.js 16 origin `Vary` ni o'chiradi. CF Response Header Transform Rule HTML ga `Vary: Accept-Language` ni **add** qiladi (`set` emas — RSC tokenlari). Mehmon kesh yo'llari (`/`, `/login`, `/register`, `/terms`, `/privacy`) chiqariladi. JSON + apply: `tools/cf-vary-accept-language.json`, `python tools/cf_vary_apply.py`. Zona qayta tuzilsa skriptni qayta yuriting |
