...
| Bosh sahifa keshi | **2026-09-19**: faqat mehmon GET `/` (query yo'q, `sessionid`/`rw_locale`/`rw:markup` yo'q). Origin `public, s-maxage=30, stale-while-revalidate=86400`; kirgan `private, no-store`. Cloudflare Cache Rule: URI Path eq `/` + GET/HEAD + `sessionid=` yo'q + `rsc` yo'q → *Eligible for cache*, *Edge TTL: Respect origin*. Worker `rankwant.uz/*` catch-all olib tashlangan — GET `/` Worker'siz (kvota). Qolgan yo'llar `rankwant.uz/a*`…`z*` va `_*`. Dashboard: **Caching → Cache Rules**. Qoida zonada, kod `apps/web/src/lib/home-cache.ts` |
