...
| 2026-09-21 | [security-suite](2026-09-21-security-suite/README.md) | `tests/security` **har PR da ishlaydi** deb yozardi, lekin yagona chaqiruvchisi (`security.yml`) **o'chirilgan** ⇒ statik yarmi (judge host qoidalari, IDOR, PAT hash, rate limit) hech qayerda avtomatik yurmasdi. Dinamik yarmining qamrovi bor edi — Nightly bake-off ayni `runner.py` ni case-filtrisiz yurgizadi (25 case, `ISOLATION_CASES` bilan) ⇒ takrorlanmaydi: yangi Nightly `security` job'i to'plamni `SECURITY_STATIC_ONLY=1` bilan chaqiradi. `gitleaks`/`pip-audit`/`npm audit` **qamrab olinmadi** — ular ham o'sha o'chirilgan workflow ichida | `tests/security/run.sh`, `nightly.yml`, `CLAUDE.md` |
| 2026-09-24 | [architecture-diagrams](2026-09-24-architecture-diagrams/README.md) · [sahifa](2026-09-24-architecture-diagrams/ARCHITECTURE.html) | Modul kesimidagi arxitektura: 20 Django modulning vazifasi/mas'uliyati, 6 oqim bosqichma-bosqich (submit→verdikt, birinchi AC→Skills→Qvant, contest, auth, guest CDN, hack routing), judge PULL shartnomasi, chegaralar. Brauzerda o'lchandi: 7 bo'lim, 6 diagramma, gorizontal toshish 0, keng SVG'lar `overflow-x:auto` ichida. `docs/06` (2026-09-06 locked) bilan farqlar ochiq yozildi: 20 vs 19 app, 10 vs 3 til, OIDC bor | `docs/06-architecture`, `apps/api`, `apps/web`, `services/judge-go` |
| 2026-09-24 | [judge-security](2026-09-24-judge-security/README.md) | Judge konteyner chegarasi **o'lchandi** (A-1 dalil): 9 konteyner bitta `rankwant_default` tarmog'ida; judge o'zidan va neytral probe'dan `postgres:5432`, `api:8000`, internet **OPEN**; `judge:8000` REFUSED (negative control). Credential qatlami butun (`DATABASE_URL` yo'q), tarmoq qatlami yo'q (`internal:` = 0). Yopish rejasi: `judge-net` + alohida `judge-queue` + `PreflightNetwork` | `docs/tasks/TASK-judge-security-a1.md` |
