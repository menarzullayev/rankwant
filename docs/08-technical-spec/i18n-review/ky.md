...
| `customizer.fontHint` | Lexend o'qish qiyinligi uchun ishlab chiqilgan. | Lexend окуу кыйынчылыгы үчүн жасалган. |  |
| `customizer.fontHeading` | Sarlavha shrifti | Аталыш шрифти |  |
| `customizer.fontHeadingHint` | Sarlavhalar uchun boshqa shrift — klassik juftlik. | Аталыштарга башка шрифт — классикалык жуп. |  |
| `customizer.font.same` | Matn bilan bir xil | Текст менен бирдей |  |
...
| `nav.menu` | Menyu | Меню |  |
| `nav.collapseSidebar` | Yon panelni yig'ish | Каптал панелди жыйноо |  |
| `nav.expandSidebar` | Yon panelni ochish | Каптал панелди ачуу |  |
...
| `footer.tagline` | O'zbekistonlik dasturchilar uchun CP/OJ platformasi | Өзбекстандык программисттер үчүн спорттук программалоо платформасы |  |
| `footer.platform` | Platforma | Платформа |  |
| `footer.contact` | Aloqa va jamiyat | Байланыш жана жамаат |  |
...
| `common.clear` | Tozalash | Тазалоо |  |
| `common.yes` | Ha | Ооба |  |
| `common.notification` | Bildirishnoma | Билдирүү |  |
| `col.time` | Vaqt | Убакыт |  |
| `col.memory` | Xotira | Эс тутум |  |
| `col.size` | Hajm | Көлөмү |  |
| `col.attempt` | Urinish | Аракет |  |
| `col.sample` | Namuna | Үлгү |  |
| `col.points` | Ball | Упай |  |
| `col.input` | Kirish | Кириш |  |
| `col.output` | Chiqish | Чыгуу |  |
| `col.weight` | Vazn | Салмагы |  |
| `col.score` | Baholash | Баалоо |  |
| `col.role` | Rol | Роль |  |
| `col.question` | Savol | Суроо |  |
| `col.options` | Variantlar | Варианттар |  |
| `col.links` | Havolalar | Шилтемелер |  |
| `col.project` | Loyiha | Долбоор |  |
| `action.save` | Saqlash | Сактоо |  |
| `action.reset` | Reset | Баштапкы абалга |  |
| `action.finalize` | Yakunlash | Аяктоо |  |
...
| `status.ok` | Muvaffaqiyat | Ийгилик |  |
| `status.warn` | Ogohlantirish | Эскертүү |  |
| `status.bad` | Xato | Ката |  |
| `status.info` | Ma'lumot | Маалымат |  |
| `status.hint.ok` | Amal muvaffaqiyatli bajarildi. | Аракет ийгиликтүү аткарылды. |  |
| `status.hint.warn` | Ehtiyot bo'ling — biror narsa chegaraga yaqin. | Этият болуңуз — бир нерсе чекке жакын. |  |
| `status.hint.bad` | Amal bajarilmadi — sababini ko'ring. | Аракет аткарылган жок — себебин көрүңүз. |  |
| `status.hint.info` | Qo'shimcha ma'lumot. | Кошумча маалымат. |  |
| `status.style.auto` | Avtomatik | Автоматтык |  |
| `status.style.autoHint` | Shaklni ekranga qarab tanlaydi: telefonda doira, kengroqda ikonka va matn. | Түрүн экранга жараша тандайт: телефондо тегерек, кененирээкте белги жана текст. |  |
| `status.style.text` | Faqat rangli matn | Түстүү текст гана |  |
| `status.style.textHint` | Rang va so'z, ikonka yo'q. Eng yengil, lekin rangni ko'rmaydiganlar uchun signal yo'q. | Түс жана сөз, белги жок. Эң жеңил, бирок түстү айырбаштабагандарга сигнал жок. |  |
| `status.style.iconText` | Ikonka va matn | Белги жана текст |  |
| `status.style.iconTextHint` | Ikonka, rang va so'z. Eng ko'p ishlatiladigan ko'rinish. | Белги, түс жана сөз. Эң көп колдонулган көрүнүш. |  |
| `status.style.badge` | To'ldirilgan nishon | Толтурулган белги |  |
| `status.style.badgeHint` | Yumshoq fonda yumaloq nishon. Ro'yxat qatorlari uchun ixcham. | Жумшак фондогу тегерек белги. Тизме саптары үчүн ыкчам. |  |
| `status.style.circle` | Doira va ikonka | Тегерек жана белги |  |
| `status.style.circleHint` | To'ldirilgan doirada oq ikonka. Eng kichik — zich jadval uchun. | Толтурулган тегеректеги ак белги. Эң кичине — тыгыз таблицалар үчүн. |  |
| `status.style.dot` | Nuqta va matn | Чекит жана текст |  |
| `status.style.dotHint` | Rangli nuqta va so'z. Fon va ramka yo'q — yengil signal. | Түстүү чекит жана сөз. Фон жана чек жок — жеңил сигнал. |  |
| `status.style.box` | Ramkali kvadrat | Чектелген квадрат |  |
| `status.style.boxHint` | Ramkali kvadrat ichida ikonka. Faqat signal, matnsiz. | Чектелген квадрат ичиндеги белги. Сигнал гана, текстсиз. |  |
| `status.style.alert` | Chap chiziq | Сол жактагы тилке |  |
| `status.style.alertHint` | Chap tomonda rangli chiziq. Sahifa yuqorisidagi xabar uchun. | Сол жакта түстүү тилке. Барактын үстүндөгү билдирүү үчүн. |  |
| `status.style.soft` | Yumshoq fonli karta | Жумшак фондуу карта |  |
| `status.style.softHint` | Yumshoq fon, ikonka, sarlavha va izoh. Forma natijasi uchun. | Жумшак фон, белги, аталыш жана эскертүү. Форма жыйынтыгы үчүн. |  |
| `status.style.outline` | Ramkali karta | Чектелген карта |  |
| `status.style.outlineHint` | Ramka, ikonka, sarlavha va izoh. Xato tafsiloti uchun. | Чек, белги, аталыш жана эскертүү. Ката чоо-жайы үчүн. |  |
| `status.style.stack` | Katta karta | Чоң карта |  |
| `status.style.stackHint` | Ikonka ustida, sarlavha va izoh ostida. Eng batafsil ko'rinish. | Белги үстүндө, аталыш жана эскертүү астында. Эң толук көрүнүш. |  |
...
| `verdict.group.ok.hint` | Yechimingiz to'g'ri — barcha testlar o'tdi. | Чечимиңиз туура — бардык тесттер өттү. |  |
| `verdict.group.warn.hint` | Hali yakunlanmagan yoki qisman bajarilgan. | Али бүтө элек же жарым-жартылай өттү. |  |
| `verdict.group.bad.hint` | Yechimingizdagi xato — tuzatib qayta yuboring. | Чечимиңизде ката бар — оңдоп кайра жөнөтүңүз. |  |
| `verdict.group.neutral.hint` | Sizning xatoyingiz emas — tizim tomonidan. Qayta urinib ko'ring. | Бул сиздин катаңыз эмес — системанын кемчилиги. Кайра аракет кылыңыз. |  |
| `verdict.group.unknown.hint` | Notanish natija kodi — tizim yangilangan bo'lishi mumkin. | Белгисиз жыйынтык коду — система жаңыртылган болушу мүмкүн. |  |
...
| `profile.activityHidden` | Faoliyat yashirilgan | Иш-аракет жашырылган |  |
| `profile.heatmapHidden` | Faollik xaritasi yashirilgan | Активдүүлүк картасы жашырылган |  |
| `profile.mapHidden` | Yechilgan masalalar xaritasi yashirilgan | Чыгарылган маселелер картасы жашырылган |  |
...
| `settings.addBadge` | Nishon qo'shish | Белги кошуу |  |
| `settings.addWebsite` | Havola qo'shish | Шилтеме кошуу |  |
| `settings.badgeColor` | Rang | Түс |  |
| `settings.badgeIcon` | Ikonka | Белги |  |
| `settings.badgeText` | Yozuv | Жазуу |  |
| `settings.badges` | Ko'nikma nishonlari | Көндүм белгилери |  |
| `settings.badgesHint` | Matn, ikonka va rang. Profilda ochiq ko'rinadi — ko'pi bilan 10 ta. | Текст, белги жана түс. Профилде ачык — эң көбү 10. |  |
| `settings.coverBuy` | Muqova sotib olish | Мукаба сатып алуу |  |
| `settings.coverOwned` | Sizda bor | Сизде бар |  |
| `settings.delivery` | Yetkazib berish manzili | Жеткирүү дареги |  |
| `settings.deliveryErase` | Manzilni o'chirish | Даректи өчүрүү |  |
| `settings.deliveryErased` | Manzil o'chirildi | Дарек өчүрүлдү |  |
| `settings.deliveryHint` | Sovg'alar uchun. Inglizcha va milliy yozuv to'liq to'ldiriladi yoki bo'sh qoladi. Profilda ko'rinmaydi. | Белектер үчүн. Англис жана төл жазууну толук толтуруңуз же бош калтырыңыз. Профилде көрүнбөйт. |  |
| `settings.endMonth` | Tugagan oyi | Бүткөн айы |  |
| `settings.firstNameEn` | Ism (inglizcha) | Аты (англисче) |  |
| `settings.gender` | Jins | Жыныс |  |
| `settings.gender.female` | Ayol | Аял |  |
| `settings.gender.male` | Erkak | Эркек |  |
| `settings.gender.non_binary` | No-binar | Бинар эмес |  |
| `settings.gender.prefer_not` | Aytmaslikni afzal ko'raman | Айтпаганды туура көрөм |  |
| `settings.genderHint` | Ixtiyoriy. Profilda ko'rsatishni o'zingiz tanlaysiz. | Милдеттүү эмес. Профилде көрсөтүүнү өзүңүз тандайсыз. |  |
| `settings.langEn` | Inglizcha | Англисче |  |
| `settings.langNative` | Milliy yozuv | Төл жазуу |  |
| `settings.lastNameEn` | Familiya (inglizcha) | Фамилиясы (англисче) |  |
| `settings.namesHint` | Inglizcha ism sertifikat va yetkazib berish uchun. Milliy ism hech qachon ommaviy emas. | Англисче ысым сертификат жана жеткирүү үчүн. Төл ысым эч качан ачык эмес. |  |
| `settings.postalAddress` | Ko'cha manzili | Көчө дареги |  |
| `settings.postalAddressNative` | Ko'cha manzili (milliy) | Көчө дареги (төл) |  |
| `settings.postalCity` | Shahar (yetkazish) | Шаар (жеткирүү) |  |
| `settings.postalCityNative` | Shahar (milliy) | Шаар (төл) |  |
| `settings.postalCode` | Pochta indeksi | Почта индекси |  |
| `settings.postalConsent` | Manzilimni sovg'a yuborish uchun ishlatishga roziman | Даректи белек жөнөтүү үчүн колдонууга макулмун |  |
| `settings.postalRecipient` | Qabul qiluvchi | Алуучу |  |
| `settings.postalRecipientNative` | Qabul qiluvchi (milliy) | Алуучу (төл) |  |
| `settings.postalRegion` | Viloyat (yetkazish) | Облус (жеткирүү) |  |
| `settings.postalRegionNative` | Viloyat (milliy) | Облус (төл) |  |
| `settings.shirtSizeEu` | Futbolka o'lchami (EU) | Футболка өлчөмү (EU) |  |
| `settings.showActivity` | Faoliyat lentasini ko'rsatish | Иш-аракет лентасын көрсөтүү |  |
| `settings.showGender` | Jinsni profilda ko'rsatish | Жынысты профилде көрсөтүү |  |
| `settings.showHeatmap` | Faollik xaritasini ko'rsatish | Активдүүлүк картасын көрсөтүү |  |
| `settings.showRecentAc` | Yechilgan masalalar xaritasini ko'rsatish | Чыгарылган маселелер картасын көрсөтүү |  |
| `settings.startMonth` | Boshlangan oyi | Башталган айы |  |
| `settings.toPresent` | Hozirgacha | Азыркыга чейин |  |
| `settings.websites` | Veb-saytlar | Веб-сайттар |  |
| `settings.websitesHint` | 2–5 ta havola. Birinchisi asosiy. https:// bilan yozing. | 2–5 шилтеме. Биринчиси негизги. https:// менен жазыңыз. |  |
...
| `settings.lastName` | Familiya | Фамилиясы |  |
| `settings.realNameHint` | Ixtiyoriy. Sertifikat va olimpiada ro'yxatlari uchun. Profilingizda ko'rinmaydi. | Милдеттүү эмес. Сертификаттар жана олимпиада тизмелери үчүн. Профилиңизде көрүнбөйт. |  |
| `settings.shirtSize` | Futbolka o'lchami | Футболканын өлчөмү |  |
| `settings.shirtSizeHint` | Ixtiyoriy. Olimpiada sovg'alari uchun. Profilingizda ko'rinmaydi. | Милдеттүү эмес. Олимпиада сыйлыктары үчүн. Профилиңизде көрүнбөйт. |  |
| `settings.gradeSchool` | Maktab | Мектеп |  |
| `settings.gradeUniversity` | Oliy ta'lim | Жогорку билим |  |
| `grade.school` | {n}-sinf | {n}-класс |  |
| `grade.bachelor` | Bakalavriat, {n}-kurs | Бакалавриат, {n}-курс |  |
| `grade.master` | Magistratura, {n}-kurs | Магистратура, {n}-курс |  |
| `grade.teacher` | O'qituvchi | Мугалим |  |
| `grade.other` | Boshqa | Башка |  |
...
| `problem.tab.mine` | Urinishlarim | Менин аракеттерим |  |
| `problem.tab.myStats` | Statistikam | Менин статистикам |  |
| `problem.tab.favourites` | Sevimlilar | Тандалмалар |  |
...
| `filter.hideSolved` | Yechilganlarni yashirish | Чечилгендерди жашыруу |  |
...
| `filter.difficultyMin` | Dan | Баштап |  |
| `filter.difficultyMax` | Gacha | Чейин |  |
| `filter.topicsInclude` | Kiradi | Кириши керек |  |
| `filter.topicsExclude` | Chiqariladi | Чыгаруу |  |
...
| `submit.expected` | Kutilgan | Күтүлгөн |  |
...
| `archive.mostLiked` | Ko'p yoqtirilgan | Көп жактырылган |  |
| `archive.tagCloud` | Mavzular | Темалар |  |
...
| `overlay.style.qogoz` | Qog'oz | Кагаз |  |
...
| `overlay.style.soyabon` | Soyabon | Кол чатыр |  |
...
| `customizer.form` | Forma | Форма |  |
| `customizer.formHint` | Matn, belgi, radio, fayl va sana shu oilaga bo'ysunadi. | Текст, белги, радио, файл жана дата ушул үй-бүлөнү ээрчийт. |  |
| `form.style.maydon` | Maydon | Талаа |  |
| `form.style.maydonHint` | Yorliq tepada. 44px maydon, 8px radius, kvadrat belgi. | Жарлык үстүндө. 44px талаа, 8px радиус, чарчы белги. |  |
| `form.style.qator` | Qator | Сап |  |
| `form.style.qatorHint` | Yorliq chapda. Switch va segment. Kapsula maydon. | Жарлык сол жакта. Которгуч жана сегмент. Капсула талаа. |  |
| `form.style.karta` | Karta | Карточка |  |
| `form.style.kartaHint` | Har tanlov — plitka. Fayl — drop zona. Oy doim ko'rinadi. | Ар тандоо — плитка. Файл — таштоо аймагы. Ай дайыма көрүнөт. |  |
| `form.style.jadval` | Jadval | Таблица |  |
| `form.style.jadvalHint` | 32px, ikki ustun, mono. Sana YYYY-MM-DD. | 32px, эки тилке, моно. Дата YYYY-MM-DD. |  |
| `form.style.orol` | Orol | Арал |  |
| `form.style.orolHint` | To'ldirilgan 22px, qattiq chiziq yo'q. Yumaloq switch. Blur kalendar. | Толтурулган 22px, катуу сызык жок. Жумуру которгуч. Булуң календарь. |  |
| `form.sample.title` | Sarlavha | Аталыш |  |
| `form.sample.body` | Matn | Текст |  |
| `form.sample.public` | Ommaviy | Ачык |  |
| `form.sample.difficulty` | Qiyinlik | Кыйындык |  |
| `form.sample.file` | Fayl | Файл |  |
| `form.sample.date` | Sana | Дата |  |
| `form.file.choose` | Tanlash | Тандоо |  |
| `form.file.empty` | Fayl tanlanmagan | Файл тандалган эмес |  |
| `form.cal.prev` | Oldingi oy | Мурунку ай |  |
| `form.cal.next` | Keyingi oy | Кийинки ай |  |
| `admin.text.correct` | To'g'ri | Туура |  |
| `admin.text.sampleTest` | Namuna test (shartda ko'rsatiladi) | Үлгү тест (шартта көрсөтүлөт) |  |
...
| `customizer.card` | Karta uslubi | Карта стили |  |
| `customizer.card.default` | Odatiy | Демейки |  |
| `customizer.card.outline` | Chegara | Чек |  |
| `customizer.card.flat` | Tekis | Түз |  |
| `customizer.card.soft` | Yumshoq soya | Жумшак көлөкө |  |
| `customizer.card.square` | Keskin burchak | Тик бурч |  |
| `customizer.cardHint` | Kontent bloklari qanday ajratiladi: chegara, soya yoki burchak. | Блоктор кантип бөлүнөт: чек, көлөкө же бурч. |  |
| `customizer.pattern` | Fon naqshi | Фон оймо-чиймеси |  |
| `customizer.pattern.none` | Yo'q | Жок |  |
| `customizer.pattern.grid` | To'r | Тор |  |
| `customizer.pattern.dots` | Nuqtalar | Чекиттер |  |
...
| `verdict.style.boxHint` | Ramkali kvadrat ichida ikonka. Faqat signal — matnsiz, lekin ajralib turadi. | Чектелген квадрат ичиндеги белги. Сигнал гана — текстсиз, бирок көзгө ташланат. |  |
| `verdict.style.bar` | Chap chiziq va nom | Сол жактагы тилке жана аталыш |  |
