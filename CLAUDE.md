...
| 2026-09-20 | **`/problems` mehmon CDN, tilga bog'liq** (HITL: problems-al). Query'siz GET; `uz` majburiy emas (`Vary: Accept-Language`). Kirgan / `rw_locale` / `?page=` / `?lang=` chiqariladi. Worker `problems*` → `problems/*` (ro'yxat kvotasiz). CF Eligible + origin TTL | `home-cache.ts`; `wrangler.toml`; `tools/cf-guest-cache-rule.json`; `tools/cf_guest_cache_apply.py`; `tools/check_decisions.py` → `problems_list_guest_cdn`; salbiy testlar 2 ta |
