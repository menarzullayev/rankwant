...
| 2026-09-20 | **TypeScript 7 yonma-yon** (HITL): typecheck native `typescript@7` (`@typescript/native`); `require("typescript")` TS 6 JS API (`@typescript/typescript6`). `typescript@7` yagona paket qilinmaydi — eslint-config-next/typescript-eslint peer `<6.1` va `i18n-runtime-hook.mjs` `transpileModule`. `@types/node` 26 alohida qadam (runtime hali Node 22) | `apps/web/package.json`; `tools/i18n-runtime-hook.mjs`; `tools/check_decisions.py` → `typescript_side_by_side`; salbiy testlar 2 ta |
