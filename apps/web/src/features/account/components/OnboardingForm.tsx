...
import { REGION_CODES, regionName } from "@rankwant/shared/regions";
import { safeNext } from "@/lib/site";
import { fieldErrors, onboardingSchema } from "@rankwant/shared/validation";
...
export function OnboardingForm({ me }: { me: Me }) {
  ...
  async function save(next: "home" | "skip") {
    setError("");
    // Tekshiruv `lib/validation.ts` da — bu forma qoidani TAKRORLAMAYDI.
    // Taxallus yagona MAJBURIY maydon (3-qaror).
    const errors = fieldErrors<"username">(
    ...
    if (errors.username) {
      setError(t(locale, errors.username));
      return;
    }
    if (next === "skip") {
