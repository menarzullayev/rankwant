...
import { strength } from "@rankwant/shared/password";
import { fieldErrors, loginSchema, registerSchema } from "@rankwant/shared/validation";
...
export function AuthForm({
  ...
  const emailStatus: FieldStatus | undefined =
    mode === "register" && email.length > 3 && fieldErrors(registerSchema.safeParse({
      identifier: "x",
      email,
      password: "dummy-pass",
      password2: "dummy-pass",
      terms_accepted: true,
    })).email
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    // Tekshiruv `lib/validation.ts` da — sahifa qoidani TAKRORLAMAYDI.
    // Sxema REJIMGA qarab tanlanadi: kirishda email va rozilik umuman
    // so'ralmaydi, ya'ni ular haqida soxta qiymat to'qib bo'lmaydi.
    const issues =
      mode === "register"
        ? fieldErrors(
