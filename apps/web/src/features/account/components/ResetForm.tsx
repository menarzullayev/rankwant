...
import { fieldErrors, resetSchema } from "@rankwant/shared/validation";
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-4">
      ...
      <p className="text-center text-theme-sm rw-dim">
        ...
        <Link href={"/login?tab=login" as Route} className="rw-accent-ink hover:underline">
          {t(locale, "auth.login")}
        </Link>
      </p>
    </form>
...
function SetNew({ token }: { token: string }) {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    // Qoida `lib/validation.ts` da — bu sahifa uni TAKRORLAMAYDI.
    const issues = fieldErrors(resetSchema.safeParse({ password, password2 }));
    const first = issues.password ?? issues.password2;
    if (first) {
