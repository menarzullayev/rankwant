...
import { padCode } from "@rankwant/shared/format";
...
export function ProblemMap({ username, problems }: { username: string; problems: ProblemTile[] }) {
  ...
  const tile = (problem: ProblemTile) => (
    <Link
      ...
      className={`flex h-7 min-w-12 items-center justify-center rounded px-1 font-mono text-theme-xs font-semibold tabular-nums transition hover:ring-2 hover:ring-[var(--rw-accent)] focus-visible:outline-2 focus-visible:outline-[var(--rw-accent-ink)] ${STATE_CLASS[problem.state]}`}
    >
